...
# Version: 0.1
#
# NODE 3D 4-dof specialization
#   3D node with 4 degrees of freedom: ux, uy, uz, p
#   (3D Cauchy u-p mixed formulation)
...
class Node34(Node):
    def __init__(self, node_id, coord=None, mass=None, fix=None):
        ...
        if len(coord) == self._nD:
            ...
            raise ValueError("oneFEM.Node34() - Number of coordinate entries does not match node dimension!")
        ...
        if len(mass) == self._nDOF:
            ...
            raise ValueError("oneFEM.Node34() - Number of mass entries does not match node number of d.o.f.s!")
        ...
        if len(fix) == self._nDOF:
            ...
            raise ValueError("oneFEM.Node34() - Number of fix entries does not match node number of d.o.f.s!")

    def __repr__(self):
        return f"Node34(ID={self._ID}, Coord={list(self._coord)}, Fix={list(self._fix)})"
    ...
    def _setDOF(self, dofs):
        if len(dofs) == self._nDOF:
            ...
            raise ValueError("oneFEM.Node34._setDOF() - Number of DOF entries does not match node number of d.o.f.s!")
    ...
    def _setFix(self, fix):
        if len(fix) == self._nDOF:
            ...
            raise ValueError("oneFEM.Node34._setFix() - Number of fix entries does not match node number of d.o.f.s!")

    def _setMass(self, mass):
        if len(mass) == self._nDOF:
            self._mass = Vector(mass, dtype=float)
        else:
            raise ValueError("oneFEM.Node34._setMass() - Number of mass entries does not match node number of d.o.f.s!")
    ...
    def _getResult(self, query, dofs):
        if query in {"force", "forces", "f"}:
            ...
            raise ValueError("oneFEM.Node34._getResult() - Unknown result type!")
