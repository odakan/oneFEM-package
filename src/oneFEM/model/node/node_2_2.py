...
# Version: 0.1
#
# NODE 2D 2-dof specialization
#   2D node with 2 degrees of freedom: ux, uy
...
class Node22(Node):
    def __init__(self, node_id, coord=None, mass=None, fix=None):
        ...
        :param mass:    Masses as a list [mass1, mass2]
        ...
        # private
        self._dofs = Vector([0, 0], dtype=int)
        self._coord = Vector([0.0, 0.0], dtype=float)
        self._mass = Vector([0.0, 0.0], dtype=float)
        self._fix = Vector([False, False], dtype=bool)

        # Initialize solution-related variables
        self._f_trial = Vector([0.0, 0.0], dtype=float)
        self._u_trial = Vector([0.0, 0.0], dtype=float)
        self._v_trial = Vector([0.0, 0.0], dtype=float)
        self._a_trial = Vector([0.0, 0.0], dtype=float)
        self._f_commit = Vector([0.0, 0.0], dtype=float)
        self._u_commit = Vector([0.0, 0.0], dtype=float)
        self._v_commit = Vector([0.0, 0.0], dtype=float)
        self._a_commit = Vector([0.0, 0.0], dtype=float)
        ...
        if len(coord) == self._nD:
            ...
            raise ValueError("oneFEM.Node22() - Number of coordinate entries does not match node dimension!")
        ...
        if len(mass) == self._nDOF:
            ...
            raise ValueError("oneFEM.Node22() - Number of mass entries does not match node number of d.o.f.s!")
        ...
        if len(fix) == self._nDOF:
            ...
            raise ValueError("oneFEM.Node22() - Number of fix entries does not match node number of d.o.f.s!")

    def __repr__(self):
        return f"Node22(ID={self._ID}, Coord={list(self._coord)}, Fix={list(self._fix)})"
    ...
    def _setDOF(self, dofs):
        if len(dofs) == self._nDOF:
            ...
            raise ValueError("oneFEM.Node22._setDOF() - Number of DOF entries does not match node number of d.o.f.s!")
    ...
    def _setFix(self, fix):
        if len(fix) == self._nDOF:
            ...
            raise ValueError("oneFEM.Node22._setFix() - Number of fix entries does not match node number of d.o.f.s!")

    def _setMass(self, mass):
        if len(mass) == self._nDOF:
            self._mass = Vector(mass, dtype=float)
        else:
            raise ValueError("oneFEM.Node22._setMass() - Number of mass entries does not match node number of d.o.f.s!")
    ...
    def _getResult(self, query, dofs):
        if query in {"force", "forces", "f"}:
            ...
            raise ValueError("oneFEM.Node22._getResult() - Unknown result type!")
