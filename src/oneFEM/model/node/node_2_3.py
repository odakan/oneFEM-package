...
# Version: 0.1
#
# NODE 2D 3-dof specialization
#   2D node with 3 degrees of freedom: ux, uy, theta_z
...
class Node23(Node):
    def __init__(self, node_id, coord=None, mass=None, fix=None):
        ...
        if len(coord) == self._nD:
            ...
            raise ValueError("oneFEM.Node23() - Number of coordinate entries does not match node dimension!")
        ...
        if len(mass) == self._nDOF:
            ...
            raise ValueError("oneFEM.Node23() - Number of mass entries does not match node number of d.o.f.s!")
        ...
        if len(fix) == self._nDOF:
            ...
            raise ValueError("oneFEM.Node23() - Number of fix entries does not match node number of d.o.f.s!")

    def __repr__(self):
        return f"Node23(ID={self._ID}, Coord={list(self._coord)}, Fix={list(self._fix)})"
    ...
    def _setDOF(self, dofs):
        ...
        if len(dofs) == self._nDOF:
            ...
            raise ValueError("oneFEM.Node23._setDOF() - Number of DOF entries does not match node number of d.o.f.s!")
    ...
    def _update(self, force, disp, vel=None, accel=None):
        ...
        :param vel: (Optional) Velocity vector
        :param accel: (Optional) Acceleration vector
        """
        self._f_trial = force
        self._u_trial = disp
        if vel is not None:
    ...
    def _setFix(self, fix):
        ...
        if len(fix) == self._nDOF:
            ...
            raise ValueError("oneFEM.Node23._setFix() - Number of fix entries does not match node number of d.o.f.s!")
    ...
    def _setMass(self, mass):
        ...
        if len(mass) == self._nDOF:
            ...
            raise ValueError("oneFEM.Node23._setMass() - Number of mass entries does not match node number of d.o.f.s!")
    ...
    def _getResult(self, query, dofs):
        ...
        if query in {"force", "forces", "f"}:
            ...
            raise ValueError("oneFEM.Node23._getResult() - Unknown result type!")
