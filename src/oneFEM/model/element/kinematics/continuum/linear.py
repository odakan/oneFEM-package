...
# Version: 0.2
#
# LinearContinuumKinematics (Option B: single object, gp-indexed)
...
class LinearContinuumKinematics(ContinuumKinematics):
    ...
    Option B: single object per element. B matrix is constant per GP and
    pre-built during initialize(). No geometric stiffness.
    """
    ...
    def __init__(self, bbar=False):
        ...
        self._bbar = bbar
        self._gp_weights = None
        self._B_bar = []

    def initialize(self, nGP, nDim, nNodes, dN_dX_list, **kwargs):
        """Store dN_dX per GP, pre-build constant B matrices.

        If bbar=True and nDim==3, also pre-builds B-bar matrices (Hughes 1980).
        Requires gp_weights kwarg from ContinuumElement._domain().
        """
        ...
        if self._bbar and nDim == 3:
            ...
            # Pass 1: element volume and mean volumetric B
            V_e = 0.0
            nVoigt = self._nVoigt
            nDOF = self._nDOF
            ...
            # Pass 2: B_bar = B_dev + B_vol_bar at each GP
            for gp in range(nGP):
                B_std = self._B[gp].data
                B_vol_gp = self._extract_B_vol(B_std)
    ...
    def copy(self):
        """Return a copy with same cached state."""
    ...
    def _buildBMatrix(dN_dX, nDim):
        ...
        Uses .data for numpy indexing internally per Design Rule 10 amendment.
        """
        dN = dN_dX.data
        nNodes = dN.shape[0]
