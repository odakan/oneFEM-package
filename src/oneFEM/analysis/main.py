...
class Analysis(object):
    def __init__(self, ID=-1, algorithm=Algorithm(), constraints=ConstraintHandler(), integrator=Integrator(), numberer=Numberer(), system=System(), test=Test()):
        # Initialize the properties
        self._ID = int(ID)                     # integer
        self._hist_steps = []                  # list of lists step history [[dt, nSteps], []]
        self._time = 0.0                       # float analysis clock (current)
        self._dt = float(0.0)                  # float last dt
        self._solution_type = None             # boolean [False:static - True:dynamic]
        self._solution_algorithm = None        # Algorithm object
        self._convergence_test = None          # Test object
        self._constraint_handler = None        # Constratints object
        self._solution_integrator = None       # Integrator object
        self._system_of_equations = None       # System object
        self._dof_numberer = None              # Numberer object
        self._is_ready_for_analysis = False    # boolean [True:analyze becomes available]
    ...
    def _test(self):
        ...
        # compute the norm
        self__norm = self._convergence_test.norm()

        # check for convergence
        if self.__norm < self._convergence_tolerance:
